...
def _generate_qr_png(text: str, scale: int = 10, border: int = 4) -> Path:
    """Generate a dense QR code PNG file for the given text and return its path."""
...
def _overlay_qr_on_pdf(input_pdf: Path, output_pdf: Path, qr_png_path: Path, qr_size_pt: float = 120.0) -> None:
    """Overlay the QR PNG on each page of the PDF and save as output_pdf.

    qr_size_pt is in PDF points (1/72 inch). 120pt ~= 1.67in square, rendered from a higher-density PNG for clarity.
    """
    try:
        with fitz.open(str(input_pdf)) as document:
            for page in document:
                ...
                label_padding = max(16.0, min(24.0, qr_size_pt * 0.18))
                label_height = 14.0
...
async def upload_pdf(file: UploadFile = File(...)) -> JSONResponse:
    ...
    _overlay_qr_on_pdf(saved_path, output_path, qr_png, qr_size_pt=120.0)
